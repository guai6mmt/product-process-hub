...
"""
import os, io, sys, csv, json, base64, sqlite3, socket, datetime, zipfile, urllib.parse, threading, webbrowser
...
def main():
    ...
    auto_open = not os.environ.get("NO_BROWSER") and "--no-browser" not in sys.argv
    # 若本系统已在另一个窗口运行（端口已在监听），直接打开页面，避免重复启动两个实例。
    probe = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    probe.settimeout(0.3)
    already = (probe.connect_ex(("127.0.0.1", PORT)) == 0)
    probe.close()
    if already:
        print("本系统已经在另一个窗口里运行。正在打开页面：%s" % url)
        if auto_open:
            try:
                webbrowser.open(url)
            except Exception:
                pass
            input("\n（如需重启：先关闭所有本系统的黑色窗口，再双击 run.bat）按回车键关闭本窗口…")
        return
